...
public static class FibonacciConsole
{
    public static void Main()
    {
        ...
        do
        {
            ...
            if (!double.TryParse(input, NumberStyles.Any, CultureInfo.CurrentCulture, out var phi)) continue;

            Console.Write("Image Count (eg 1000):");
            input = Console.ReadLine();
            if (!int.TryParse(input, NumberStyles.Any, CultureInfo.CurrentCulture, out var imageCount)) continue;
    ...
    private static async Task GenerateImageSequences(string tempImagePath, double phi, int imageCount,
        CancellationToken token)
    {
        ...
        for (var i = 0; i < imageCount; i++)
        {
            ...
            await Task.Run(
                () => ImageGenerator.ExportSequence(sequence, imagePath, fileFormat, ImageSize, ImageSize, PointSize),
                token);
        }
    }
}