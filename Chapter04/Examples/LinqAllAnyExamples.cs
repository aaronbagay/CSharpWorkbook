...
class LinqAllAnyExamples
{
    public static void Main()
    {
        ...
        var summary = string.Join(" | ", hand.OrderByDescending(c => c.Number)
            .Select(c => c.ToString()));
        Console.WriteLine($"Hand: {summary}");
        
        Console.WriteLine($"Any Clubs: {hand.Any(card => card.Suit == PlayingCardSuit.Clubs)}");
        Console.WriteLine($"Any Red: {hand.Any(card => card.Suit is PlayingCardSuit.Hearts or PlayingCardSuit.Diamonds)}");
